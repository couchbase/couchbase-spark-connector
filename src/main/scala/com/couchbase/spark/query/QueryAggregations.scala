...
object QueryAggregations extends Logging {
  ...
  /** Converts a spark aggregation into a sequence of fields. If empty the aggregation
   * is not supported or something else went wrong.
   *
   * @param agg the aggregation to convert.
   * @return a non-empty sequence if supported and present.
   */
  def convertAggregateExpressions(agg: Aggregation, schema: StructType): Seq[StructField] = {
    agg.aggregateExpressions.map({
      ...
      case avg: Avg =>
        ...
        StructField(s"AVG($distinct`$fieldName`)", original.dataType, original.nullable, original.metadata)
      case _: CountStar =>
