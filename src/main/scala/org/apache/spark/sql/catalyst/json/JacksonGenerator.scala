...
/**
  ...
  *
  * Couchbase note: See comments on [[JacksonParser]] for why this Spark code is C&Ped into the connector.
  */
class JacksonGenerator(
  ...
  require(dataType.isInstanceOf[StructType] || dataType.isInstanceOf[MapType]
    ...
    s"JacksonGenerator only supports to be initialized with a ${StructType.simpleString}, " +
      s"${MapType.simpleString} or ${ArrayType.simpleString} but " +
  ...
  private val gen = {
    ...
    if (options.pretty) {
      generator.setPrettyPrinter(
        new DefaultPrettyPrinter(PrettyPrinter.DEFAULT_SEPARATORS.toString))
    }
  ...
  private def makeWriter(dataType: DataType): ValueWriter = dataType match {
    ...
    // VariantType support is not included to reduce dependencies
  ...
  def write(row: InternalRow): Unit = {
    writeObject(writeFields(
      fieldWriters = rootFieldWriters.toSeq,
      row = row,
