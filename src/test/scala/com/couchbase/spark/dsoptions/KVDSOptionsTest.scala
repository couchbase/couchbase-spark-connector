...
class KVDSOptionsTest {
  ...
  def testKVGet(): Unit = {
    ...
    val result = spark
      .sparkContext
      .couchbaseGet(Seq(Get("airport::sfo")),connectionOptions = connectionProps)
      .collect()

    assertEquals(1, result.length)
    assertEquals("SFO", result.head.contentAs[JsonObject].get.get("iata"))
  }
