...
class RequiresOperationalCluster extends ExecutionCondition {
  ...
  override def evaluateExecutionCondition(context: ExtensionContext): ConditionEvaluationResult = {
    ...
    if ("operational" == clusterType) {
      ConditionEvaluationResult.enabled("Test is enabled for 'operational' cluster type")
    } else {
      ConditionEvaluationResult.disabled("Test is disabled for non-'operational' cluster type")
    }
  }
}