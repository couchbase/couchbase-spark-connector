...
/** Tests multiple cluster connections where they are dynamically setup, against a regular config,
  * with SparkSQL operations.
  */
class MultiClusterConnectionDynamicSQLIntegrationTest extends SparkOperationalTest {
  ...
  def missingUsernameAndPassword(): Unit = {
    val id =
      s"couchbase://${infra.params.connectionString}?spark.couchbase.implicitBucket=${infra.params.bucketName}"
    runStandardSQLQuery(spark, id)
  }

  @Test
  def minimalSettings(): Unit = {
    val id = s"couchbase://${infra.params.connectionString}"
    runStandardSQLQuery(spark, id)
  }
  ...
  def missingImplicitBucket(): Unit = {
    val id =
      s"couchbase://${infra.params.username}:${infra.params.password}@${infra.params.connectionString}"
    runStandardSQLQuery(spark, id)
  }
}
