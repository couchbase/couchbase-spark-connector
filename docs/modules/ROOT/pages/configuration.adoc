...
val spark = SparkSession
  .builder()
  .appName("MultipleClusterConnections")
...
NOTE: This feature is available with Spark Connector 3.5.0 and later.

Individual SQL++ queries can be partitioned into multiple sub-queries, which can then be executed in parallel by multiple Spark workers.

Partitioning is enabled by specifying all four partition options (if one is specified, all must be):
...
----
    val airports = spark.read
      ...
      .option(QueryOptions.PartitionLowerBound, "0")
      .option(QueryOptions.PartitionUpperBound, "10000")
      .option(QueryOptions.PartitionCount, "10")
      .load()
----

This will generate and execute 10 sub-queries similar to:

* SELECT [...] WHERE [...] AND id < 1000
* SELECT [...] WHERE [...] AND (id >= 1000 AND id < 2000)
* SELECT [...] WHERE [...] AND (id >= 2000 AND id < 3000)
* ...
* SELECT [...] WHERE [...] AND id >= 9000
...
Best results will be achieved when these are chosen carefully to ensure that each partition is a similar size.
