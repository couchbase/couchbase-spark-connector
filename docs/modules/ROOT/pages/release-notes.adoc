...
== Couchbase Spark Connector 3.3.4 GA (8 January 2023)
...
When performing a DataFrame SQL++ read (`spark.read().format("couchbase.query")`), rows will now be streamed and backpressured, rather than buffered in-memory.
